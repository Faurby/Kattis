...
public class BipartiteXCustom {
     ...
     * @param  G the graph
     */
    public BipartiteXCustom(GraphCustom G) {
        isBipartite = true;
    ...
    private void bfs(GraphCustom G, int s) { 
        ...
        while (!q.isEmpty()) {
            int v = q.dequeue();
            if(G.adjConflict(v) != null){
                for (int w : G.adjConflict(v)) {
                    if (!marked[w]) {
                        marked[w] = true;
                        edgeTo[w] = v;
                        color[w] = !color[v];
            ...
            if (G.adjHarmony(v) != null){
                for (int w : G.adjHarmony(v)) {
                    if (!marked[w]) {
                        marked[w] = true;
                        edgeTo[w] = v;
                        color[w] = color[v];
    ...
    private boolean check(GraphCustom G) {
        // graph is bipartite
        if (isBipartite) {
            for (int v = 0; v < G.V(); v++) {
                for (int w : G.adj(v)) {
